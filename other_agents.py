...
def get_fcfs_action(machine, job_slot1, pa):
    act = len(job_slot1.slot)  # if no job can be scheduled, hold
    
    for i in xrange(len(job_slot1.slot)):
        new_job = job_slot1.slot[i]
        
        if new_job is not None:  # There is a pending job
            # Check resource availability for the cloud machine (for long or expensive jobs)
            avbl_res_cloud = machine.avbl_slot1[:new_job.len, :]
            res_left_cloud = avbl_res_cloud - new_job.res_vec
            
            # Check resource availability for the edge machines (for normal jobs)
            avbl_res_edge1 = machine.avbl_slot2[:new_job.len, :]
            res_left_edge1 = avbl_res_edge1 - new_job.res_vec
            
            avbl_res_edge2 = machine.avbl_slot3[:new_job.len, :]
            res_left_edge2 = avbl_res_edge2 - new_job.res_vec

            # Cloud node for long or expensive jobs
            if np.all(res_left_cloud[:] >= 0) and (
                new_job.len >= pa.anomalous_job_len_middle_bound or 
                np.any(new_job.res_vec >= pa.anomalous_job_resources_lower)):
                act = i 
                return act 
            
            # Edge node for normal jobs
            elif new_job.len < pa.anomalous_job_len_middle_bound and np.all(new_job.res_vec < pa.anomalous_job_resources_lower):
                if np.all(res_left_edge1[:] >= 0):
                    act = len(job_slot1.slot) + i + 1  # Schedule on first edge machine
                    return act 
                elif np.all(res_left_edge2[:] >= 0):
                    act = len(job_slot1.slot) * 2 + i + 1  # Schedule on second edge machine
                    return act

    return act


def get_wrr_action(machine, job_slot1, pa):
    node_weights = [0.5, 0.25, 0.25]
    total_weight = sum(node_weights)
    selection_probabilities = [weight / total_weight for weight in node_weights]

    # Schedule jobs based on WRR probabilities
    for job_index in range(len(job_slot1.slot)):
        new_job = job_slot1.slot[job_index]
        if new_job is not None:
            selected_node = np.random.choice(3, p=selection_probabilities)

            if selected_node == 0:  # Cloud node
                avbl_res_cloud = machine.avbl_slot1[:new_job.len, :]
                res_left_cloud = avbl_res_cloud - new_job.res_vec
                if np.all(res_left_cloud[:] >= 0):
                    act = job_index 
                    return act

            elif selected_node == 1:  # Edge node 1
                avbl_res_edge1 = machine.avbl_slot2[:new_job.len, :]
                res_left_edge1 = avbl_res_edge1 - new_job.res_vec
                if np.all(res_left_edge1[:] >= 0):
                    act = len(job_slot1.slot) + job_index + 1 
                    return act

            elif selected_node == 2:  # Edge node 2
                avbl_res_edge2 = machine.avbl_slot3[:new_job.len, :]
                res_left_edge2 = avbl_res_edge2 - new_job.res_vec
                if np.all(res_left_edge2[:] >= 0):
                    act = len(job_slot1.slot) * 2 + job_index + 1
                    return act

    return len(job_slot1.slot)