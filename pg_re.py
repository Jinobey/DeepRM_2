...
def init_accums(pg_learner):  # in rmsprop
    ...
    #RMSPROp is adaptive learning rate optimization algo to train NN. Good against vanishing gradient.
    accums = []
...
#TODO figure out the diff between this func and the one above.
    ...
    for iteration in xrange(1, pa.num_epochs):
        ...
        with open('./metrics.csv', 'a') as f:
                ...
                if os.stat('./metrics.csv').st_size == 0:
                    writer.writerow(['name', 'value', 'timestep', 'step'])
                writer.writerow(['NumTrajs', len(eprews), 0 , iteration])
                writer.writerow(['NumTimesteps', np.sum(eplens), 0 , iteration])
                writer.writerow(['MaxRew', np.average([np.max(rew) for rew in all_eprews]), 0 , iteration])
                writer.writerow(['MeanRew_lower', np.std(eprews), 0 , iteration])
                writer.writerow(['MeanRew_upper', np.mean(eprews), 0 , iteration])
                writer.writerow(['MeanSlowdown', np.mean(all_slowdown), 0 , iteration])
                writer.writerow(['MeanLen+', np.mean(eplens), 0 , iteration])
                writer.writerow(['MeanLen-', np.std(eplens), 0 , iteration])
                writer.writerow(['MeanEntropy', np.mean(all_entropy), 0 ,iteration])
