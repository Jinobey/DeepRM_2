...
def launch(pa, pg_resume=None, render=False, repre='image', end='all_done', use_cnn=True):
    ...
    for iteration in xrange(1, pa.num_epochs):
        ...
        with open('./metrics.csv', 'a') as f:
            ...
            writer.writerow(['MeanRew_lower', float(np.std(eprews)), 0, iteration])
            writer.writerow(['MeanRew_upper', float(np.mean(eprews)), 0, iteration])
            writer.writerow(['MeanSlowdown', float(np.mean(all_slowdown)), 0, iteration])
